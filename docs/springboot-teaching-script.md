...
至于 web.xml、spring-mvc.xml 那 80 多行——那是 Web 层的配置。demo1 没有 Web 模块，不需要它们。第三章加了 Web Starter 之后，我们会看到 Spring Boot 如何处理这些。

### 1.4 Spring Boot 的答案："约定大于配置"

**配置**的意思是：你告诉框架每一个细节——数据源类名、端口号、视图解析器、扫描路径。

**约定**的意思是：框架预设了一套默认行为。你按规矩来，细节不用说明。不搞特殊就自动生效。

举例：你引入 Spring Boot，不写端口配置。启动后 8080 端口已经在监听。想改？`server.port=9090` 一行。没写？默认 8080。

把每个人都要写一遍的东西变成默认就有的。想改可以改，不改就能用。这就是约定大于配置。
...
Spring Boot = Spring + Spring MVC + 自动配置 + 内嵌服务器。它是 Spring 的"自动挡"——手动挡的技能全有效，只是不用踩离合了。
...
回到第一章看过的 demo1。启动类在 `com.example.demo1`，@Service 在 `com.example.demo1.service.impl`——在子包下，自动被扫描。构造器注入的 @Autowired 自动生效。运行 `mvn spring-boot:run`，0.4 秒启动，IoC 和 AOP 全在工作。

这个工程没有 Web 能力。下一步——加一个依赖。
...
### 6.3 配置中心 Nacos Config ——商家后台统一改价
